...
from uMux_IF_Chain.devices import sts31_dis
...
class UMux_IF_Rev2(uMux_IF_Board):
    ...
    def nulling_enable(self) -> None:
        data = [0x00] * _CMD.CMD_LEN
        data[0] = (_CMD.NULLING_CTRL << 1) | _CMD.W
        data[1] = 0x01
        self._write(data)
        time.sleep(self._delay)
        ret = self._read(_RET_VAL.RET_LEN)
    ...
    def nulling_state_get(self) -> bool:
        data = [0x00] * _CMD.CMD_LEN
        data[0] = (_CMD.NULLING_CTRL << 1) | _CMD.R
        self._write(data)
        time.sleep(self._delay)
        ret = self._read(_RET_VAL.RET_LEN)
        if(ret[0] & _RET_VAL.MASK_READ_GOOD):
    ...
    def nulling_up_set(self, dac_val_I: int, dac_val_Q: int) -> None:
        # Keep in mind that a zero voltage is mid scale (0x8000)
    ...
    def synth_set_Frequency_MHz(self, Freq_MHz) -> float:
        ...
        if(real_freq_MHz != Freq_MHz) and (self.debug > 1):
    ...
    def read_temperatures_C(self) -> tuple[float, float]:
        ...
        ret = self._read(_RET_VAL.RET_LEN)
        if(ret[0] & _RET_VAL.MASK_READ_GOOD):
            synth_temp_C = self._tmp.convert_int2temp_C(ret[1:3])
            mcu_temp_C = self._tmp.convert_int2temp_C(ret[3:5])
            return (synth_temp_C, mcu_temp_C)
        else:
            print("Read Local Tempereatures Failed")
            return (-1, -1)
    ...
    def _write_eeprom(self, bsn, mcu_pn, freq_range, mixer_pn, synth_pn, bb_pn, lo_leak_pn):
        if(len(bsn) > 16):
            raise ValueError("BSN is too long")
        
        if(len(mcu_pn) > 16):
            raise ValueError("mcu_pn is too long")

        if(len(freq_range) > 16):
            raise ValueError("freq_range is too long")

        if(len(mixer_pn) > 16):
            raise ValueError("mixer_pn is too long")

        if(len(synth_pn) > 16):
            raise ValueError("synth_pn is too long")

        if(len(bb_pn) > 16):
            raise ValueError("bb_pn is too long")
        
        if(len(lo_leak_pn) > 16):
            raise ValueError("lo_leak_pn is too long")
